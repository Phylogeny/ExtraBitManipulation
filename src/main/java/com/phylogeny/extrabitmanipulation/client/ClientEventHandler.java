...
//	private static final int[] SHAPE_FLAT = new int[]{3, 3, 3, 4, 5, 6, 3}; TODO
	...
	public void preventArmorRendering(RenderPlayerEvent.Pre event)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (stackVanity != null && stack != null)
			{
				armor[i] = stack;
	...
	public void onKeyInput(@SuppressWarnings("unused") InputEvent.KeyInputEvent event)
	{
				...
				|| KeyBindingsExtraBitManipulation.OPEN_CHISELED_ARMOR_SLOTS_GUI.isKeyDown())
		{
			...
			{
				if (stack != null && stack.hasTagCompound() && ItemStackHelper.isDesignStack(stack))
					openBitMappingGui();
			}
	...
	public void throwBit(TickEvent.PlayerTickEvent event)
	{
		...
		if (stack != null && (ChiselsAndBitsAPIAccess.apiInstance.getItemType(stack) == ItemType.BIT_BAG
								...
								else
								{
									...
									else if (!shiftDown || removeBits || drawnMode)
									{
										...
										else
										{
											...
											swingTool = ((ItemSculptingTool) item).sculptBlocks(stack, player, player.worldObj, pos, side, hit, drawnStartPoint, sculptingData);
				...
				if (bitLoc != null)
				{
					...
					if (configBox.renderInnerShape || configBox.renderOuterShape)
					{
						...
						if (configBox.renderOuterShape)
							RenderGlobal.func_189697_a(box.expandXyz(BOUNDING_BOX_OFFSET).offset(-playerX, -playerY, -playerZ),
	...
	{
		RenderGlobal.func_189697_a(boxBounding, red, green, blue, outerAlpha / 255.0F);
		GlStateManager.depthFunc(GL11.GL_GREATER);
		RenderGlobal.func_189697_a(boxBounding, red, green, blue, 28 / 255.0F);
		GlStateManager.depthFunc(GL11.GL_LEQUAL);
	}
