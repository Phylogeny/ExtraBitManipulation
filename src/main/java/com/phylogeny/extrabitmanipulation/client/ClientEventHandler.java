...
//	private static final int[] SHAPE_FLAT = new int[]{3, 3, 3, 4, 5, 6, 3}; TODO
	...
	public void interceptMouseInput(MouseEvent event)
	{
		...
		else if (event.button == 0)
		{
			...
			{
				Item item = stack.getItem();
				if (event.buttonstate && ItemStackHelper.isBitWrenchItem(item))
				{
					event.setCanceled(true);
				}
	...
	public void renderBoxesSpheresAndOverlays(RenderWorldLastEvent event)
	{
		if (!Configs.disableOverlays)
		{
			...
			{
				MovingObjectPosition target = Minecraft.getMinecraft().objectMouseOver;
				if (target != null && target.typeOfHit.equals(MovingObjectType.BLOCK) && ItemStackHelper.isBitToolItem(stack.getItem()))
				{
					IChiselAndBitsAPI api = ChiselsAndBitsAPIAccess.apiInstance;
						...
						for (int s = 0; s < 6; s++)
						{
							if (s != side)
							{
								...
								if (mode2 != 2 || oppRotation)
									renderTexturedSide(t, wr, s, northSouth, box, minU, maxU, minV, maxV, 1);
								...
								if (configBox.renderInnerShape || configBox.renderOuterShape)
								{
									...
									if (mode == 0)
									{
										...
										AxisAlignedBB box2 = !removeBits ? new AxisAlignedBB(pos2.getX(), pos2.getY(), pos2.getZ(),
												pos2.getX() + 1, pos2.getY() + 1, pos2.getZ() + 1) : block.getSelectedBoundingBox(world, pos2);
...
//			int rotation = dir / 6;
	...
	private AxisAlignedBB contractBoxOrRenderArrows(boolean contractBox, Tessellator t, WorldRenderer wr, int side, boolean northSouth, EnumFacing dir,
