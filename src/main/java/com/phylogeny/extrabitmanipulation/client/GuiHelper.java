...
public class GuiHelper
{
			...
			Vec3d rotationVec, float scale, float scaleMax, float rotationMultiplier, boolean affectRotation)
	{
		...
		if (clickedMouseButton == 0)
		{
			...
			{
				double angleX = rotationVec.x - (deltaY / scale) * rotationMultiplier;
				double angleY = rotationVec.y - (deltaX / scale) * rotationMultiplier;
		...
		{
			double offsetX = translationInitialVec.x - deltaX;
			if (offsetX < -OFFSET_MAX || offsetX > OFFSET_MAX)
				offsetX = OFFSET_MAX * (offsetX > 0 ? 1 : -1);
			
			double offsetY = translationInitialVec.y - deltaY;
	...
	public static Pair<Vec3d, Float> scaleObjectWithMouseWheel(GuiScreen screen, AxisAlignedBB box, Vec3d translationVec, float scale, float scaleMax)
	{
		...
		int x = mouseX - (int) (translationVec.x + box.maxX / 2.0F + box.minX / 2.0F);
		int y = mouseY - (int) (translationVec.y + box.maxY / 2.0F + box.minY / 2.0F);
