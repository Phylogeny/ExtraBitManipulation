	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		for (int i = 0; i < entryCount; i++)
		{
			...
			if (mouseY >= bitMappingList.top && mouseY <= bitMappingList.bottom)
			{
				...
				{
					drawHoveringText(Arrays.<String>asList(new String[] {EnumChatFormatting.DARK_RED + (buttonStates.selected ? "State" : "Block")
							+ ": " + EnumChatFormatting.RESET + (buttonStates.selected ? entry.getState().toString()
		...
		for (int i = 0; i < tabButtons.length; i++)
		{
			if (tabButtons[i].isMouseOver())
				drawHoveringText(Arrays.<String>asList(new String[] {tabButtons[i].getHoverText()}),
						mouseX, mouseY, mc.fontRendererObj);
		}
	}
