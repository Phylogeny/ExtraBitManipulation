...
public class LayerChiseledArmor implements LayerRenderer<EntityLivingBase>
{
			...
			float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
	{
		...
		if (headStack != null && headStack.hasTagCompound() && headStack.getItem() instanceof ItemChiseledArmor)
		{
		...
		if (chestStack != null && chestStack.hasTagCompound() && chestStack.getItem() instanceof ItemChiseledArmor)
		{
			GlStateManager.pushMatrix();
			...
			renderSleeve(displayLists[1], EnumHandSide.RIGHT, scale);
			renderSleeve(displayLists[2], EnumHandSide.LEFT, scale);
			GlStateManager.popMatrix();
		}
		ItemStack legsStack = entity.getItemStackFromSlot(EntityEquipmentSlot.LEGS);
		if (legsStack != null && legsStack.hasTagCompound() && legsStack.getItem() instanceof ItemChiseledArmor)
		{
			GlStateManager.pushMatrix();
		...
		if (feetStack != null && feetStack.hasTagCompound() && feetStack.getItem() instanceof ItemChiseledArmor)
		{
			GlStateManager.pushMatrix();
	...
	{
		renderArmorPiece(rightLeg, displayListRight, scale, offsetY);
	...
	private void renderArmorPiece(ModelRenderer modelArmorPiece, int displayList, float scale, float offsetY)
	{
		...
		renderArmorPiece(displayList, scale, 0.0F, offsetY);
	}
