...
public class ItemStackHandlerArmorItem extends ItemStackHandler
{
	...
	public ItemStackHandlerArmorItem(ItemStack[] stacks, NBTTagCompound nbt, int partIndex, boolean clientSide)
	{
		super(stacks);
	...
	public ItemStack removeSlot(int slotIndex)
	{
		...
		{
			if (slotIndex != i)
				stacks[i > slotIndex ? i - 1 : i] = this.stacks[i];
		}
		this.stacks = stacks;
		return stack;
	}
