...
public class EntityBit extends EntityThrowableFixed implements IEntityAdditionalSpawnData
{
	...
	protected void onImpact(RayTraceResult result)
	{
		...
		else if (result.typeOfHit == RayTraceResult.Type.BLOCK)
		{
			...
			if (!(isLava ? Configs.disableIgniteBlocks : Configs.disableExtinguishBlocks) && !drop)
			{
				if (!worldObj.isRemote)
				{
					...
					else
					{
						...
						{
							playSound(SoundEvents.BLOCK_FIRE_EXTINGUISH, 0.5F, 2.6F + (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.8F);
			...
			if (!worldObj.isRemote)
			{
				float volume = MathHelper.sqrt_double(motionX * motionX + motionY * motionY + motionZ * motionZ) * 0.2F;
				if (volume > 1.0F)
					volume = 1.0F;
			...
			try
			{
				...
				IBitAccess bitAccess = api.getBitAccess(worldObj, pos);
				if (api.canBeChiseled(worldObj, pos))
				{
